...
# Mask the sky out
def mask_sky(rgb_image):
    lower_sky_brightness = np.array([150,180,200])
    upper_sky_brightness = np.array([250,250,250])
    
    mask = cv2.inRange(rgb_image, lower_sky_brightness, upper_sky_brightness)
...
def create_feature(rgb_image):
    ...
    # Make sure the image is 32 x 32
    assert rgb_image.shape[0] == 32 and rgb_image.shape[1] == 32
    
    ## TODO: Convert image to HSV color space
    hsv = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2HSV)
      
    # Mask the image
    # The traffic sign will be assigned to 0
    # exept the traffic lights and sky
    mask = cv2.inRange(hsv[:,:,2], 35, 200)
    masked_image = np.copy(rgb_image)
    masked_image[mask != 0] = [0, 0, 0]
    
    # Crop image to get rid of the sky on the
    # top, left, right, bottom cornors
    cropped_image = masked_image[3:30, 5:25]
    
    # Remove the sky or white spaces from the cropped image
    no_sky_image = mask_sky(cropped_image)
                               
    ## TODO: Create and return a feature value and/or vector
    feature = no_sky_image
    # plt.imshow(feature)
...
# 
# Frist I tried to detect the brightness of the image using hsv, then used the v to cast out the traffic light box leaving only the traffic light. 
# Then tried to cast out the sky from the rgb image leaving traffic light image with black background.
# After that I used RGB features to distiguish between red, yellow and green lights (It was still difficult for me to separate between red and yellow lights sometime when both red and yellow lights are bright.
# 
...
def estimate_label(rgb_image):
    ...
    # Classify using get_avg of red, green, blue method 
...
# estimate_label(STANDARDIZED_LIST[729][0])
# estimate_label(STANDARDIZED_LIST[730][0])
# estimate_label(STANDARDIZED_LIST[735][0])
...
# 
# Classifying between red and yellow lights when both red and yellow lights are bright.
